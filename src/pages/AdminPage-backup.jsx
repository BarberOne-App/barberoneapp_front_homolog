...
const barberNames = useMemo(() => {
  const allNames = [...new Set(allPaid.map(p => p.appointment?.barber?.displayName || 'Sem barbeiro'))];
  
  if (isAdmin) {
    return allNames;
  } else {
    return allNames.filter(name => name === currentUser?.name || name === currentUser?.displayName);
  }
  ...
  const handleAddBlockedDate = async () => {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL}/blocked-dates`, { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify(blockData) });
      if (response.ok) {
  ...
  const handleRemoveBlockedDate = (id) => {
    showConfirm('Deseja realmente desbloquear esta data?', async () => {
      try {
        const response = await fetch(`${import.meta.env.VITE_API_URL}/blocked-dates/${id}`, { method: 'DELETE', headers: { Authorization: `Bearer ${token}` } });
        if (response.ok) {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      for (const s of toExpire) {
        await fetch(`${import.meta.env.VITE_API_URL}/subscriptions/${s.id}`, {
          method: 'PATCH',
      ...
      try {
        const valesRes = await fetch(`${import.meta.env.VITE_API_URL}/employeeVales`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
        const paymentsRes = await fetch(`${import.meta.env.VITE_API_URL}/employeePayments`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
  ...
  const getAppointmentByPayment = (payment) => {
    ...
    return appointments.find(
      (apt) => apt.id?.toString() === appointmentId?.toString()
    ) || null;
  };
  ...
  //   }
  //   try {
  //     await fetch(`${import.meta.env.VITE_API_URL}/users/${selectedUserPermissions.id}/permissions`, {
  //       method: 'PATCH',
  //       headers: {
  //         'Content-Type': 'application/json',
  //         Authorization: `Bearer ${token}`
  ...
  try {
    const response = await fetch(`${import.meta.env.VITE_API_URL}/users/${selectedUserPermissions.id}/permissions`, {
      method: 'PATCH',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${token}`
    ...
    if (!response.ok) {
      ...
      throw new Error(errorData.message || 'Erro ao salvar permissões');
    }

    // Recarrega a lista de usuários e outros dados
    await loadData();

    // Se o usuário alterado for o próprio logado, atualizar a sessão
    if (selectedUserPermissions.id === currentUserRef.current?.id) {
      const userResponse = await fetch(`${import.meta.env.VITE_API_URL}/users/${selectedUserPermissions.id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (userResponse.ok) {
        const updatedUser = await userResponse.json();
        // Atualiza o ref e o localStorage
        currentUserRef.current = updatedUser;
        localStorage.setItem('session', JSON.stringify(updatedUser));

        showToast('Permissões atualizadas. A página será recarregada para aplicar as alterações.', 'success');
    ...
    console.error(error);
    showToast(error.message || 'Erro ao salvar permissões.', 'danger');
  }
};
  ...
  const handleSaveBarber = async (e) => {
    ...
    try {
      ...
      if (editingBarber?.isUserOnly) {
        ...
        await fetch(`${import.meta.env.VITE_API_URL}/users/${editingBarber.userId}`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
      ...
      if (barberForm.createUser && !editingBarber) {
        ...
        // const checkEmailResponse = await fetch(`${import.meta.env.VITE_API_URL}/users`, {
        //   headers: {
        //     Authorization: `Bearer ${token}`,
        //   },
        // });
    ...
    showConfirm('Deseja realmente excluir este funcionário?', async () => {
      try {
        if (isUserOnly) {
          ...
          if (barber?.userId) {
            await fetch(`${import.meta.env.VITE_API_URL}/users/${barber.userId}`, {
              method: 'DELETE',
              headers: {
                Authorization: `Bearer ${token}`,
                'Content-Type': 'application/json'
  ...
  const confirmCancelSubscription = async () => {
    ...
    try {
      ...
      await fetch(`${import.meta.env.VITE_API_URL}/subscriptions/${sub.id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      const userRes = await fetch(`${import.meta.env.VITE_API_URL}/users/${sub.userId}`);
      if (userRes.ok) {
 ...
 */
  ...
  const handleSavePlan = async (e) => {
    ...
    try {
      ...
      if (editingPlan) {
        const response = await fetch(`${import.meta.env.VITE_API_URL}/subscription-plans/${editingPlan.id}`, {
          method: 'PATCH',
          headers: {
            Authorization: `Bearer ${token}`,
            'Content-Type': 'application/json',
  ...
  const handleSaveBenefit = async (e) => {
    ...
    try {
      ...
      await fetch(`${import.meta.env.VITE_API_URL}/subscription-plans/${plan.id}`, {
        method: 'PATCH',
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const handleSaveVale = async (e) => {
    ...
    try {
      ...
      await fetch(`${import.meta.env.VITE_API_URL}/employeeVales`, {
        method: 'POST',
        headers:
        {
          Authorization: `Bearer ${token}`,
      ...
      const res = await fetch(`${import.meta.env.VITE_API_URL}/employeeVales`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const handleDeleteVale = (id) => {
    showConfirm('Deseja excluir este vale?', async () => {
      await fetch(`${import.meta.env.VITE_API_URL}/employeeVales/${id}`, { method: 'DELETE' });
  ...
  const handleSaveExtraPayment = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL}/employeePayments`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
  ...
  const handleMarkPayrollPaid = async (employee, barberData, period) => {
    ...
    showConfirm(confirmMsg, async () => {
      try {
        ...
        await fetch(`${import.meta.env.VITE_API_URL}/employeePayments`, {
          method: 'POST', headers: {
            Authorization: `Bearer ${token}`,
            'Content-Type': 'application/json'
          }, body: JSON.stringify(paymentRecord)
        });
        for (const v of vales) await fetch(`${import.meta.env.VITE_API_URL}/employeeVales/${v.id}`, {
          method: 'DELETE',
          headers: {
            Authorization: `Bearer ${token}`,
            'Content-Type': 'application/json'
        ...
        if (barberId) {
          ...
          await Promise.all(toMark.map(p =>
            fetch(`${import.meta.env.VITE_API_URL}/appointmentPayments/${p.id}`, {
              method: 'PATCH',
              headers: {
                Authorization: `Bearer ${token}`,
                'Content-Type': 'application/json'
        ...
        const [valesRes, paymentsRes, aptsRes] = await Promise.all([
          fetch(`${import.meta.env.VITE_API_URL}/employeeVales`),
          fetch(`${import.meta.env.VITE_API_URL}/employeePayments`),
          fetch(`${import.meta.env.VITE_API_URL}/appointmentPayments`, {
            headers: {
              Authorization: `Bearer ${token}`,
              'Content-Type': 'application/json'
            }
                      ...
                      <div className="fluig-barber-stats">
                        <div className="stat-item">
                          ...
                          <span className="stat-value">
                            ...
                            {planCount > 0 && <span style={{ color: '#d4af37', fontSize: '0.75rem', marginLeft: 4 }}>({planCount} plano)</span>}
                          </span>
                        </div>
                        <div className="stat-item">
                            ...
                            <tbody>
                              {[...barberPayments]
                                ...
                                .map((payment) => {
                                  ...
                                  const productsList = appointment?.products?.filter(pr => pr && pr.productName) || [];
                                  const hasProducts = productsList.length > 0;
                                  const productsTotal = productsList.reduce((s, prod) => {
                                    const price = typeof prod.unitPrice === 'string' ? parseFloat(prod.unitPrice.replace(/R\$/g, '').replace(/,/g, '.').trim()) || 0 : prod.unitPrice || 0;
                                  ...
                                  return (
                                    <tr key={payment.id}>
                                      <td>{new Date(payment.appointment.startAt).toLocaleDateString('pt-BR')}</td>
                                      <td>
                                      ...
                                      <td>
                                        {hasProducts
                                          ? productsList.map((prod, idx) => (
                                            <div key={idx} style={{ fontSize: '0.85rem' }}>{prod.productName} <span style={{ color: '#888' }}>x{prod.quantity || 1}</span></div>
                                          ))
                                          : <span style={{ color: '#555' }}>—</span>}
                                      </td>
                                      <td>{new Date(payment.paidAt).toLocaleDateString('pt-BR')} : <span style={{ color: '#666' }}>—</span></td>
                                      <td>
                                        {planOnly
                ...
                <div className="form-section">
                  ...
                  <div style={{ marginBottom: '1rem' }}>
                    ...
                    <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'flex-start' }}>
                      {homeInfo.heroImage && (
                        <img
                          src={homeInfo.heroImage} alt="Preview do banner"
                                ...
                                <tr key={payment.id}>
                                  ...
                                  <td data-label="Para">
                                    {(() => {
                                      ...
                                      return la?.dependent ?
                                        <span style={{
                                          display: 'inline-flex', alignItems: 'center', gap: '4px', background: 'rgba(255,122,26,0.12)',
                                          color: '#ff7a1a', border: '1px solid rgba(255,122,26,0.35)', borderRadius: '20px', padding:
                                            '2px 9px', fontSize: '0.75rem', fontWeight: 700
                                        }}>
                                          👤 {la.dependent.name}</span>
                                        : <span style={{ color: '#555' }}>—</span>;
                                    })()}
                                  </td>
                                  <td data-label="Barbeiro">{payment.appointment.barber.displayName}</td>
                      ...
                      <table>
                        ...
                        <tbody>
                          {getFilteredAppointmentPayments()
                            ...
                            .sort((a, b) => (b.startAt || b.paidAt || '').localeCompare(a.startAt || a.paidAt || ''))
                            .map((payment) => {
                              const time = new Date(payment.appointment.startAt).toLocaleTimeString('pt-BR', {
                                hour: '2-digit',
                                minute: '2-digit',
                                timeZone: 'UTC',
                              });
                              ...
                              const appointment = payment.appointmentId
                                ...
                                : appointments.find(apt =>
                                  ...
                                  apt.date === payment.startAt &&
                                  apt.time === payment.time
                                );
                              const productsList = appointment?.products?.filter(pr => pr && pr.productName) || [];

                              const productsTotal = productsList.reduce((sum, prod) => {
                                const price = typeof prod.unitPrice === 'string'
                                  ? parseFloat(prod.unitPrice.replace(/R\$/g, '').replace(/,/g, '.').trim()) || 0
                                  : prod.unitPrice || 0;
                              ...
                              return (
                                <tr key={payment.id}>
                                  <td data-label="Data">
                                    {new Date(payment.appointment.startAt).toLocaleDateString('pt-BR')} {time}
                                  </td>
                                  ...
                                  <td data-label="Para">
                                    {appointment?.dependent ? (
                                      ...
                                      }}>👤 {appointment.dependent.name}</span>
                                    ) : (
                    ...
                    <table>
                      ...
                      <tbody>
                        {[...pendingPayments].sort((a, b) => (b.appointmentDate || b.createdAt || '').localeCompare(a.appointmentDate || a.createdAt || '')).map((payment) => (
                          ...
                          >
                            <td>{new Date(payment.appointment.startAt).toLocaleDateString('pt-BR')}</td>
                            <td>{new Date(payment.appointment.startAt).toLocaleTimeString('pt-BR', {
                              hour: '2-digit',
                              minute: '2-digit',
                              timeZone: 'UTC'
                            })}</td>
                            ...
                            <td>
                              {(() => { const la = appointments.find(a => a.id?.toString() === payment.appointmentId?.toString()); return la?.dependent ? <span style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', background: 'rgba(255,122,26,0.12)', color: '#ff7a1a', border: '1px solid rgba(255,122,26,0.35)', borderRadius: '20px', padding: '2px 9px', fontSize: '0.75rem', fontWeight: 700 }}>👤 {la.dependent.name}</span> : <span style={{ color: '#555' }}>—</span>; })()}
                            </td>
                ...
                <tbody>
                  {[...barberPayments]
                    ...
                    .map((payment) => {
                      ...
                      return (
                        <tr key={payment.id}>
                          <td>{payment.appointment?.startAt ? new Date(payment.appointment.startAt).toLocaleDateString('pt-BR') : '—'}</td>
                          <td>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
              ...
              <div style={{ marginBottom: '1rem' }}>
                ...
                <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'flex-start' }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    >
                      {productImageUploading ? '⏳ Enviando...' : '🖼️ ' + (productForm.imageUrl ? 'Alterar imagem' : 'Enviar imagem')}
                    </label>
                    <input
